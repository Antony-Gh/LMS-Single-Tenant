...
import 'package:esoi/app/widgets/main_widget/assignment_widget/assignment_history_widget.dart';
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      ...
      if (!isInstructor) {
        if (assignment.userStatus == 'passed') {
          showSnackBar(ErrorEnum.success, appText.assignmentPassed,
              desc: appText.assignmentPassedDesc);
        } else if (assignment.userStatus == 'not_passed') {
          showSnackBar(ErrorEnum.error, appText.assignmentClosed,
              desc: appText.assignmentPassedDesc);
        }
      }
                  ...
                  child: Column(
                    children: [
                      ...
                      ...List.generate(historyData.length, (index) {
                        return AssignmentHistoryWidget.message(
                            historyData[index]);
                      }),
                      space(150),
                    ],
                        ...
                        child: Row(
                          children: [
                            Expanded(
                              child: button(
                                  onTap: () async {
                                    bool? res = await AssignmentHistoryWidget
                                        .newReplaySheet(
                                            assignment.assignmentId!,
                                            assignment.student!.id!);

                                    if (res != null && res) {
                                      getData(assignment.assignmentId!,
                                          assignment.student!.id!);
                                    }
                                  },
                                  width: getSize().width,
                            ...
                            if (isInstructor) ...{
                              ...
                              Expanded(
                                child: button(
                                    onTap: () async {
                                      bool? res = await AssignmentHistoryWidget
                                          .setGradeSheet(assignment.id!,
                                              assignment.passGrade!.toString());
