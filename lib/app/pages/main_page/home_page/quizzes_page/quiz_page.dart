...
import 'package:esoi/app/models/quize_model.dart';
import 'package:esoi/app/services/user_service/quiz_service.dart';
import 'package:esoi/app/widgets/main_widget/quizzes_widget/quiz_widget.dart';
...
import 'package:esoi/common/utils/date_formater.dart';
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) {
      if (((ModalRoute.of(context)!.settings.arguments as List)[0])
        ...
        isStartQuiz = true;
      } else if (((ModalRoute.of(context)!.settings.arguments as List)[0])
          is Quiz) {
  ...
  startTimer() {
    ...
    timer = Timer.periodic(const Duration(seconds: 1), (timer) {
      if ((quizTime?.inSeconds ?? -1) >= 1) {
        progressValue =
            ((quizTime?.inSeconds ?? 0) / ((quizData?.time ?? 0) * 60));

        quizTime = Duration(seconds: quizTime!.inSeconds - 1);
        seconds =
            int.parse(formatHHMMSS(quizTime?.inSeconds ?? 0).split(':').last);
        setState(() {});
      } else {
        print('........end........');
        timer.cancel();
                            ...
                            child: Column(
                              ...
                              children: [
                                ...
                                if (isReview) ...{
                                  ...
                                  // timer
                                  QuizWidget.timer(
                                      progressValue, quizTime, seconds),
                                },

                                // question and counter
                                QuizWidget.questionProgressBar(
                                              ...
                                              return GestureDetector(
                                                ...
                                                child: QuizWidget.multiAnswerItem(
                                                    quizData!
                                                        ...
                                                        .answers![index]!,
                                                    isReview,
                                                    userAnswer: reviewQuizData
                                                                ?.answersheet
                                                                ?.items
                                                                .keys
                                                                .toList()
                                                                .singleWhere((element) => quizData!.questions![currentQuestionIndex].id.toString() == element,
                                                                    orElse: () =>
                                                                        '-') ==
                                                            '-'
                                                        ? null
                                                        : reviewQuizData?.answersheet?.items[reviewQuizData
                                                            ?.answersheet
                                                            ?.items
                                                            .keys
                                                            .toList()
                              ...
                              onTap: () async {
                                if (isReview) {
                                  if (beforTabPage == 'MyResults') {
                                    ...
                                    bool res = await QuizService.storeReview(
                                        reviewQuizData!.id!,
                                        quizData!.questions!);
                                  ...
                                  bool res = await QuizService.storeResult(
                                      quizData!.id!,
                                      quizResultId!,
                                      quizData!.questions!);
