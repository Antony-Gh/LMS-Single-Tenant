...
import 'package:esoi/app/models/single_content_model.dart';
...
class _OfflineSingleContentPageState extends State<OfflineSingleContentPage> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((timeStamp) async {
      ...
      if (content != null) {
        singleContent = await AppDataBase.getListOfSingleContentDataAtDB(
            courseId!, content!.id!);
      }
          ...
          Positioned.fill(
            ...
            child: Column(
              ...
              children: [
                ...
                if ((singleContent?.storage == 'upload' ||
                        singleContent?.storage == 'external_link' ||
                        singleContent?.storage == 's3') &&
                    videoFormats
                        .contains(singleContent?.fileType?.toLowerCase())) ...{
                  space(20),
                  CourseVideoPlayer(
                    singleContent?.file ?? '',
                    '',
                    ...
                    isLoadNetwork: false,
                    localFileName: singleContent?.file?.split('/').last ??
                        '${singleContent?.title}.${singleContent?.fileType}',
                  ),
                },
                  ...
                  child: Wrap(
                    ...
                    children: [
                      ...
                      if (singleContent?.date != null) ...{
                        SingleCourseWidget.courseStatus(
                            appText.startDate,
                            timeStampToDate((singleContent?.date ?? 0) * 1000)
                                .toString(),
                      ...
                      if (singleContent?.createdAt != null) ...{
                        SingleCourseWidget.courseStatus(
                            appText.publishDate,
                            timeStampToDate(
                                    (singleContent?.createdAt ?? 0) * 1000)
                                .toString(),
                            AppAssets.calendarSvg,
                            width: getSize().width * .38),
                      },
