...
class RegisterDropDownWidget extends StatefulWidget {
  ...
  const RegisterDropDownWidget(this.setData, this.title, this.options,
      {super.key});
    ...
    return directionality(
      ...
      child: dropDown(
          ...
          List.generate(widget.options.length,
              (index) => widget.options[index].getTitle()), () {
        // onTapOpenBox
        setState(() {
          isOpen = !isOpen;
        });
      ...
      }, isOpen, icon: AppAssets.infoCircleSvg, isBorder: false),
    ));
  }
}
