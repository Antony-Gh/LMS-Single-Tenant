...
import 'package:esoi/app/models/assignment_model.dart';
import 'package:esoi/app/models/instructor_assignment_model.dart';
...
class AssignmentService {
  ...
  static Future<InstructorAssignmentModel?>
      getAllAssignmentsInstructor() async {
    try {
  ...
  static Future<List<AssignmentModel>> getStudents(int assignmentId) async {
    ...
    try {
      String url =
          '${Constants.baseUrl}instructor/assignments/$assignmentId/students';
  ...
  static Future<bool> setGrade(int historyId, int grade) async {
    try {
      String url =
          '${Constants.baseUrl}instructor/assignments/histories/$historyId/rate';
      print(url);

      Response res = await httpPostWithToken(url, {"grade": grade});
  ...
  static Future<List<ChatModel>> getHistory(
      int assignmentId, int studentId) async {
    List<ChatModel> data = [];

    try {
      String url =
          '${Constants.baseUrl}panel/assignments/$assignmentId/messages?student_id=$studentId';
  ...
  static Future<bool> newQuestion(
      int id, String fileTitle, String desc, File? file, int studentId) async {
    try {
      ...
      dio.FormData formData = dio.FormData.fromMap({
        ...
        if (file != null) ...{
          "attachment": await dio.MultipartFile.fromFile(file.path,
              filename: file.path.split('/').last),
        }
      });

      dio.Response res =
          await dioPostWithToken(url, formData, isRedirectingStatusCode: false);

      if (res.data['success']) {
        ErrorHandler()
            .showError(ErrorEnum.success, res.data, readMessage: true);
        return true;
      } else {
