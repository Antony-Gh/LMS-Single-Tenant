...
class MeetingService {
  ...
  static Future<bool> createLink(int id, String url, String password) async {
    try {
      ...
      Response res = await httpPostWithToken(
          url, {"link": url, "reserved_meeting_id": id, "password": password});
  ...
  static Future<bool> addSession(int id) async {
    try {
      ...
      if (jsonResponse['code'] == 200) {
        showSnackBar(ErrorEnum.success, appText.liveSessionCreated,
            desc: appText.youCanJoinItNow);
        return true;
      } else {
        ErrorHandler()
            .showError(ErrorEnum.error, jsonResponse, readMessage: true);
  ...
  static Future<bool> finisheMeeting(int id, bool isConsultant) async {
    try {
      String url =
          '${Constants.baseUrl}${isConsultant ? 'instructor/meetings/$id/finish' : 'panel/meetings/$id/finish'}';
