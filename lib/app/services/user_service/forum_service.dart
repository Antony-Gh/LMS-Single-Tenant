...
class ForumService {
  ...
  static Future<bool> answerResolve(int id) async {
    try {
      String url =
          '${Constants.baseUrl}panel/webinars/forums/answers/$id/resolve';
  ...
  static Future<bool> setAnswer(int id, String desc) async {
    try {
      ...
      Response res = await httpPostWithToken(url, {"description": desc},
  ...
  static Future<bool> newQuestion(
      int id, String title, String desc, File? file) async {
    try {
      String url = '${Constants.baseUrl}panel/webinars/$id/forums';
