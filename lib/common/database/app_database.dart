...
class AppDataBase {
  static Future getCoursesAndSaveInDB() async {
    ...
    if (purchaseData.isNotEmpty) {
      // open course box
      Box<CourseModelDB> courseBox =
          await Hive.openBox<CourseModelDB>('courseBox');

      for (var i = 0; i < purchaseData.length; i++) {
        print('course: ${i + 1}');

        int courseId = purchaseData[i].webinar == null
            ? purchaseData[i].bundle!.id!
            : purchaseData[i].webinar!.id!;

        if (purchaseData[i].bundle == null) {
          await getCourseAndSave(courseBox, courseId,
          ...
          for (var j = 0; j < bundleWebinars.length; j++) {
            ...
            await getCourseAndSave(courseBox, bundleWebinars[j].id!,
  ...
  static Future getCourseAndSave(
      Box<CourseModelDB> courseBox, int courseId, String courseJson) async {
  ...
  static Future<List<CourseModel>> getCoursesAtDB() async {
    ...
    for (var i = 0; i < courseBox.values.length; i++) {
      CourseModel course = CourseModel.fromJson(jsonDecode(
  ...
  static Future<List<ContentModel>> getContentsCoursesAtDB(int courseId) async {
    ...
    if (courseDB != null) {
      ...
      for (var i = 0; i < (contents.length); i++) {
        data.add(ContentModel.fromJson(contents[i]));
      }
  ...
  static Future<SingleContentModel?> getListOfSingleContentDataAtDB(
      int courseId, int contentId) async {
    Box<CourseModelDB> courseBox =
        await Hive.openBox<CourseModelDB>('courseBox');
    CourseModelDB? courseDB = courseBox.get(courseId);
